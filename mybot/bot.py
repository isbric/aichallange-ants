...
class MyBot:
	...
	def do_turn(self, ants):
		self.debug_turn = self.debug_turn + 1
		...
		for ant in ants.my_ants():
			for other in self.my_ants:
				if ant =
			(x for x in ants.my_ants if x not in lambda pos: pos.pos):
		...
		self.debug('Starting turn\n')
		# ant logic
		for ant in ants.my_ants():
			hill_distances = []
			for hill in ants.my_hills():
				self.debug('TEST\n')
				self.debug('ants.cols:{}, ants.rows:{}, ant:{}, hill:{}, distance:{}'.format(ants.cols, ants.rows, ant, hill, self.distance(10, 10, (1,1), (2,2))))
			...
			if (len(hill_distances) and hill_distances[0][1] < 4):	# Move away from hills or stay put if you cant
				...
				self.debug(', choice: away from other ants, or stay.')
				if not self.move_away_from_target(ant, ant_distances[0][0]):
					self.debug(', choice: stay.')

			elif (len(ants.food())):
				self.debug(', choice: get food.')
				...
				self.debug(', choice: find food, random move.')
				directions = ('n', 'e', 's', 'w')
				for direction in directions:
